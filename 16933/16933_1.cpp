...
// 이 문제에서 ‘상태(state)’는 좌표 (x, y)과 ‘k개중 몇개의 벽을 부쉈는지 여부’와 '밤인지 낮인지 여부' 이며, 각 상태에서 필요한 정보는 ‘최단 거리’뿐이므로, dist[x][y][k] 형태의 3차원 배열이면 충분하다. 
...
int main() {
    ...
    int ans = -1;
    while(!q.empty()) {
        auto [curX,curY,curK,curD] = q.front(); q.pop();
        if(curX == n-1 && curY == m-1) {
            ans = dist[curX][curY][curK][curD];
            break;
        }
        for(int dir = 0; dir < 4; dir++) {
            ...
            if(board[nx][ny] == 1) {
                if(curD == DAY) nk += 1; // DAY, Wall Breaks Count < k -> Using Wall Breaks Count
                else nx = curX, ny = curY; // NIGHT, Wait
            }
            // visited
            if(dist[nx][ny][nk][nd]) continue;
